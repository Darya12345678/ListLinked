...
public void addLast(E e){
    Node<E> prev = lstNode;                 //создали указатель на объект в строке 7
    prev.setCurrentElement(e);               //задали ему текущий элемент соответ-но остались без последнего элемента
    lstNode = new Node<E>(null, prev, null); // заново создали последний элемент(указ-ль на пред-щий эл.теперь prev)
    prev.setNextElement(lstNode);            //задаем предп-му эл-ту указатель на созданный выше пустой элемент
    size++;
}
@Override
...
private class Node<E>{
    ...
    public Node<E> gtPrevElement(){return prevElement;}
